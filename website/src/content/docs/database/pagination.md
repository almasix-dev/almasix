...
Almasix's paginators work with the [query builder](/database/queries/) and [Articulate](/articulate/), and know the URL they were reached at, so they can write the links to the pages either side of them.
...
The page comes from the request's `page` query parameter, so a controller that wants Laravel's behaviour says nothing at all. Pass one to override it:

```python
page = await DB.table("users").order_by("id").paginate(15, page=2)
```

If you already know the total — a count you cached, say — hand it over and the count query is skipped:
...
`Model.per_page` decides the size when you do not pass one; it is 15 unless the model says otherwise.

### Simple pagination

When the view only needs "Previous" and "Next", `simple_paginate` skips the count query. It reads one row more than a page and reports whether it found it:
...
A cursor is where the last page ended: the ordered columns of its final row, base64-encoded into the URL. The page after it is a `WHERE` comparison rather than an `OFFSET`, which means rows inserted while a reader is paging do not shift the window, and the database does not count past what it returns.
...
The ordering is what the cursor is made of, so the query needs one; several `order_by` clauses page correctly, compared column by column:

```python
posts = await Post.query().order_by("author").order_by("title", "desc").cursor_paginate(15)
```

The cursor is read from the request's `cursor` parameter, or handed over directly:

```python
page = await Post.query().order_by("id").cursor_paginate(15, previous_page.next_cursor())
```

The cost is that there are no page numbers and no jumping about: a cursor paginator can only go forwards and back.
...
```python
# resources/views/users/index.prism.html
...
`links()` renders `pagination.tailwind` (or `pagination.simple-tailwind` for a simple paginator). Both ship with the framework; write `resources/views/pagination/tailwind.prism.html` in your application to replace one, or name another view:

```python
users.links("pagination.my-own")
```

Bootstrap 5 views ship too. Say so once, in a service provider, and every paginator uses them:
...
A long run of pages is elided at both ends, three either side of the current page. `on_each_side` widens or narrows that window:
...
`with_path` says where the pages live when the paginator is built somewhere the request cannot be read:
...
A paginator serializes to the shape Laravel's does, so an API client written against one reads the other:
...
A cursor paginator has no page numbers to report, so its JSON carries the cursors instead:
...
A cursor paginator adds `cursor`, `next_cursor()` and `previous_cursor()`, each a `Cursor` — `encode()` writes one into a URL and `Cursor.from_encoded()` reads it back.
