...
from almasix.prism.compiler import DirectiveHandler
from almasix.prism.engine import ComposerCallback, Engine
...
def get_engine() -> Engine:
    if _engine is None:
        raise RuntimeError("Prism engine is not set. Bootstrap the Application first.")
    return _engine
...
def view(
    ...
    *,
    ...
    """Render a Prism template to an HTML response."""
...
def render(name: str, data: dict[str, Any] | None = None) -> str:
    """Render a Prism template to a string (no HTTP wrapper)."""
