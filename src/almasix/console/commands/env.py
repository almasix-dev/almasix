...
Almasix's encrypter offers one authenticated cipher and no choice of algorithm,
...
from almasix.encryption.encrypter import Encrypter, generate_key
from almasix.encryption.exceptions import DecryptException

#: Where ``env:decrypt`` looks when no ``--key`` is given.
KEY_VARIABLE = "ALMASIX_ENV_ENCRYPTION_KEY"
...
class EnvironmentFileCommand(Command):
    ...
    def root(self) -> Path:
        """The working directory, as ``smith`` was run — not the stale app root."""
        return Path.cwd()
