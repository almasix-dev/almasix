...
from almasix.console.scheduling.event import Event, call_hook
...
INTERRUPT_KEY = "almasix:schedule:interrupt"
...
def _invoke(event: Event, runner: Runner | None) -> int:
    if event.callback is not None:
        ...
        if inspect.isawaitable(result):
            # Almasix's ORM and queue are awaitable, so a scheduled callback is
...
def _dispatch_job(event: Event) -> int:
    from almasix.queue.helpers import dispatch as dispatch_job
...
def _mail_output(event: Event, output: str, code: int) -> None:
    ...
    if event.email_only_on_failure and code == 0:
        return
    try:
        from almasix.console.scheduling.mail import mail_output
...
def _report(exception: BaseException) -> None:
    try:
        from almasix.support.helpers import report
