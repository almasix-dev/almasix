...
from almasix.console.kernel import ConsoleKernel, DiscoveryFailure


class FrontDoor:
    """Keeps a Typer app pointed at the commands of the current directory.

    A ``smith`` process usually runs one command in one directory and exits, so
    a single kernel would do. But the CLI is also driven in-process — by tests,
    and by anything embedding Smith — where the directory moves under it. So the
...
def install(typer_app: typer.Typer, *, cwd: Path | None = None) -> ConsoleKernel:
    """Register every command Smith can reach from ``cwd`` onto ``typer_app``."""
...
def report_failures(kernel: ConsoleKernel) -> None:
    ...
    the notice is printed whatever you typed — not only on ``smith list``.
    """
