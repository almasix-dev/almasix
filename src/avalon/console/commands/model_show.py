...
introspection commands do: a fact Avalon cannot check is left out rather than
...
from avalon.console.repl import discover_app_classes
from avalon.orm.model import EVENTS, Model, RelationDescriptor
...
class ModelShowCommand(Command):
    ...
    Laravel also names the model's policy. Avalon does not, because finding
    ...
    def _resolve(self, name: str) -> type[Model] | None:
        ...
        if not (isinstance(found, type) and issubclass(found, Model)):
            self.error(f"[{name}] is not an Avalon model.")
            return None
        return found
    ...
    def _app_models(self) -> dict[str, type[Model]]:
        """The application's models, found the way Fiddle finds them."""
        return {
            name: found
