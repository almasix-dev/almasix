...
Avalon caches one of them, and the reason is the runtime rather than an
...
#: What Laravel caches that Avalon does not, and why. Printed by ``optimize``
...
class ViewCacheCommand(Command):
    ...
    description = "Compile every Caliburn template"
    ...
    def handle(self) -> int:
        ...
        deploy. Caliburn compiles to Python functions held by the engine, so
...
class ViewClearCommand(Command):
    ...
    description = "Drop the compiled Caliburn templates"
...
class OptimizeCommand(Command):
    signature = "optimize"
    description = "Cache what Avalon can cache, and say what it deliberately does not"
    ...
    def handle(self) -> int:
        ...
        boot it would otherwise repeat. An Avalon process boots once and serves
