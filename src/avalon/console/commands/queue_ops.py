...
from avalon.queue.restart import broadcast_restart, restart_store
...
class QueueRestartCommand(Command):
    ...
    def warn_when_signal_cannot_travel(self) -> None:
        ...
        interpreter, so the timestamp is gone the moment ``grail`` exits.
        """
        from avalon.cache.drivers.array import ArrayStore
        from avalon.cache.helpers import get_manager
