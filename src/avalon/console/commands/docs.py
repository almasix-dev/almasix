"""``docs`` — where Avalon's documentation is.

Avalon's documentation site is not published yet; M39 publishes it. This
command therefore has no URL to open by default and does not invent one. It
opens ``$AVALON_DOCS_URL`` when that names a site you can reach — your own
...
DOCS_URL_VARIABLE = "AVALON_DOCS_URL"

#: The documentation sources in the Avalon repository. They ship with no wheel,
...
class DocsCommand(Command):
    ...
    description = "Open Avalon's documentation in a browser"
    ...
    def report_unpublished(self, page: str) -> None:
        ...
        self.warn("Avalon's documentation site is not published yet.")
        self.line(f"Read it in the Avalon repository, under {source}")
