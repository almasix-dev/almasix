...
application, so they keep working in a directory that ``avalon new`` has not
touched yet.
"""
...
from avalon.grail.make import MakeError, make, make_component, make_view, view_name
from avalon.orm.inflector import snake, studly, table_name
from avalon.orm.migration import MigrationError, make_migration
from avalon.support.str import Str
...
class Generator(Command):
    ...
    #: Key into ``avalon.grail.make.BLUEPRINTS``.
    kind: str = ""
    ...
    def root(self) -> Path:
        """Where the file lands: the working directory, as ``grail`` was run."""
        return Path.cwd()
...
class MakeViewCommand(Command):
    ...
    description = "Create a Caliburn view in resources/views"
    boots_application = False
...
class MakeComponentCommand(Command):
    ...
    description = "Create an anonymous Caliburn component in resources/views/components"
    boots_application = False
