...
driver Avalon supports, so those rows are always real. Custom types and the
...
from avalon.orm.connection import Connection, ConnectionError_
from avalon.orm.dialects import quote_ident
...
_NO_SESSION_COUNT: dict[str, str] = {
    ...
    "oracle": "Oracle keeps this in v$session, which needs privileges Avalon cannot assume",
}
...
class DbTableCommand(DatabaseIntrospectionCommand):
    ...
    is how :meth:`Command.choice` answers everywhere in Grail.
    """
...
class DbMonitorCommand(DatabaseIntrospectionCommand):
    ...
    ``DatabaseBusy`` event: Avalon has no such event to listen for, and the
...
def _pool_note(connection: Connection) -> str:
    """Why this driver has no session count, and what Avalon can see instead."""
    reason = _NO_SESSION_COUNT.get(
        connection.dialect, f"Avalon knows no session count for {connection.dialect}"
    )
    return (
