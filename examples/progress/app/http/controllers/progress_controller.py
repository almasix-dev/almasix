    ...
    return [
        ...
        },
        {
            ...
            "proof": [
                ...
                "{{ app_name }} → controller data key (complete + definition)",
                "route('') / vite('') argument names; ctrl+space anywhere in a template",
                "@if → snippet keeps @, closes with @endif, aligns under indent",
                "textDocument/formatting → format_prism (Ctrl+Alt+L / format-on-save)",
                "env('APP_') / ${APP_} keys from .env + config; DB.table / .where columns",
                "user.email / post.title attributes from migrations + models",
                "string completions keep surrounding quotes (textEdit range)",
        ...
        },
        {
            ...
            "proof": [
                ...
                "@almasix/sonar (almasix-dev/sonar / npm)",
