...
def run(
    command_cls: type, app: Any, capsys: pytest.CaptureFixture[str], **input: Any
) -> tuple[int, str]:
...
def test_cache_clear_reports_a_store_that_will_not_flush(
    app: Any, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]
) -> None:
...
def test_cache_forget_removes_one_key_and_leaves_the_rest(
...
def test_cache_forget_says_so_when_the_key_was_never_there(
    app: Any, capsys: pytest.CaptureFixture[str]
) -> None:
...
def test_view_cache_reports_a_template_that_will_not_compile(
    app: Any, capsys: pytest.CaptureFixture[str]
) -> None:
...
def test_view_clear_drops_the_compiled_templates(
    app: Any, capsys: pytest.CaptureFixture[str]
) -> None:
...
def test_the_view_commands_report_an_application_without_an_engine(
...
def test_optimize_caches_views_and_names_what_it_does_not_cache(
    app: Any, capsys: pytest.CaptureFixture[str]
) -> None:
...
def test_optimize_clear_clears_both_the_cache_and_the_views(
    app: Any, capsys: pytest.CaptureFixture[str]
) -> None:
    Cache.put("kept", "value")
    engine = app.make(Engine)
...
def test_optimize_clear_fails_when_a_step_fails(
    app: Any, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]
) -> None:
    monkeypatch.setattr(type(Cache.store()), "flush", lambda self: False)
